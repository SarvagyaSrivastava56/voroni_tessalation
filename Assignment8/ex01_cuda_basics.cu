...
__global__ void squaredDiff(const float *A, const float *B, float *C, int N) {
  ...
  if (i < N) {
     ...
     * ══════════════════════════════════════════════════════ */
    C[i] = (A[i] - B[i]) * (A[i] - B[i]); /* <-- replace this line */
  }
}
...
void diy_launch_config(void) {
  ...
  for (int c = 0; c < n_cases; c++) {
     ...
     * ══════════════════════════════════════════════════════ */
    int blocks = (N + threads - 1) / threads; /* <-- replace with the correct formula */
...
void diy_memory_bandwidth(void) {
  ...
  for (int s = 0; s < n_sizes; s++) {
    ...
    /* ══════════════════════════════════════════════════════
     * TODO (B4-D2H): Measure Device->Host transfer time.
     *   HINT: Same pattern but cudaMemcpyDeviceToHost direction.
     * ══════════════════════════════════════════════════════ */
    cudaEventRecord(t0);
...
int main(void) {
  ...
  printf("  GPU: %s  (SM %d.%d)  VRAM: %.0f MB\n\n", prop.name, prop.major,
         prop.minor, prop.totalGlobalMem / 1e6);
